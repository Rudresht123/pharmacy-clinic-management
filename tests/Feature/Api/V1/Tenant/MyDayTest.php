...
class MyDayTest extends TenantTestCase
{
    ...
    /**
     * The day is spread across its hours by booked time, and only this
     * doctor's bookings are in it.
     */
    public function test_the_day_is_spread_across_its_hours(): void
    {
        [$organization, $ids, $branchId] = $this->clinic();
        ...
        $customers = $this->onTenant($organization, fn () => [
            ...
            Customer::on('organization')->create(['name' => 'Bhavin Shah', 'phone' => '9810000002'])->id,
            Customer::on('organization')->create(['name' => 'Chetna Rao', 'phone' => '9810000003'])->id,
            Customer::on('organization')->create(['name' => 'Dev Kumar', 'phone' => '9810000004'])->id,
        ]);

        $this->book($customers[0], $ids['anjali'], $branchId, '10:00');
        $this->book($customers[1], $ids['anjali'], $branchId, '10:15');
        $this->book($customers[2], $ids['anjali'], $branchId, '11:00');

        // Somebody else's, at an hour Anjali has nobody: it must not appear.
        $this->book($customers[3], $ids['vikram'], $branchId, '12:00');

        $this->signInAsDoctor($organization, 'anjali@clinic.test');

        $byHour = $this->getJson('/api/v1/tenant/opd/my-day?date='.self::MONDAY)
            ->assertOk()
            ->json('data.by_hour');
        ...
        $this->assertSame(0, $byHour['walk_ins']);
    }

    /**
     * A fortnight ending on the day asked about: evenly spaced, the empty days
     * sent as zero, and only this doctor's patients counted.
     */
    public function test_the_trend_is_a_fortnight_of_the_doctors_own_days(): void
    {
        [$organization, $ids, $branchId] = $this->clinic();
        ...
        $seen = $this->book($customers[0], $ids['anjali'], $branchId, '10:00');
        $this->book($customers[1], $ids['anjali'], $branchId, '10:15');
        $this->book($customers[2], $ids['vikram'], $branchId, '10:30');
        ...
        // Thirteen days before, through the day itself.
        $this->assertSame('2026-08-25', $trend[0]['date']);
        $this->assertSame(0, $trend[0]['total']);

        $this->assertSame(self::MONDAY, $trend[13]['date']);
        $this->assertSame(2, $trend[13]['total']);
        $this->assertSame(1, $trend[13]['seen']);
    }
