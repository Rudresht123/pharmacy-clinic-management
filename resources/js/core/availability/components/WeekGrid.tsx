...
const STATE: Record<DayState, { label: string; tone: string }> = {
    ...
    none: { label: 'No schedule', tone: 'none' },
};
...
export function WeekGrid({
    ...
    /** Filtered by the page; the week's own list when nothing is filtered. */
    doctors: WeekDoctor[];
                    ...
                    <thead>
                        <tr>
                            ...
                            {week.days.map((day) => (
                                <th
                                    key={day.date}
                                    className={day.is_today ? 'is-today' : undefined}
                                >
                                    <span>{day.label}</span>
                                    <small>{day.day}</small>
                                </th>
                        ...
                        {doctors.map((doctor) => (
                            ...
                            >
                                <th scope="row" className="wk-who">
                                    {/*
                                        The whole name cell selects the row. A
                                        row is scanned across and then clicked
                                        at whichever end the eye stopped, and a
                                        hit target the width of an avatar is a
                                        target people miss.
                                    */}
                                    <button
                                        type="button"
                                ...
                                {doctor.days.map((cell) => (
                                    <td key={cell.date}>
                                        {cell.state === 'none' ? (
                                            /*
                                                A dash, not an empty cell.
                                                Blank reads as "not loaded"; a
                                                dash says somebody looked and
                                                there is nothing.
                                            */
                                            <span className="wk-none" aria-label="No schedule">
                                                —
                                            </span>
            ...
            */}
            <ul className="wk-legend">
                {(Object.keys(STATE) as DayState[]).map((state) => (
                    <li key={state}>
                        {state === 'none' ? (
                            <span className="wk-key is-none" aria-hidden="true">
                                —
                            </span>
                        ) : (
